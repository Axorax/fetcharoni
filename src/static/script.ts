...
document.getElementById('import-froni-input')!.addEventListener('change', function (this: HTMLInputElement) {
		...
		if (extension === 'froni') {
			...
			reader.onload = function (event) {
				if (event.target) {
					...
					parseFroniFile(JSON.parse(target.result as string));
				}
			};

			reader.readAsText(file);
		} else {
			alert('Please select a ".froni" file.');
		}
	}
});
...
function parseFroniFile(data: any = { address: '', method: 'GET' }): void {
...
function importRequest(): void {
	...
	setImport(null);
	parseFroniFile(importParsers[importType](importInput.innerText));
}
...
document.querySelectorAll('div[contenteditable]').forEach((div) => {
	div.addEventListener('keydown', function (e) {
		...
		} else if (keyEvent.key === '"' || keyEvent.key === "'" || keyEvent.key === '(' || keyEvent.key === '{' || keyEvent.key === '[' || keyEvent.key === '<') {
			keyEvent.preventDefault();
			const pairMap = {
				'"': '"',
				"'": "'",
